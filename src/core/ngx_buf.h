...
struct ngx_buf_s {
    ...
    off_t            file_last;//处理文件时，待处理文件结尾标记

    u_char          *start;         /* start of buffer */
    u_char          *end;           /* end of buffer */
    ngx_buf_tag_t    tag;      //缓冲区标记地址 void类型指针
    ...
    unsigned         recycled:1; //标志位 为1 可回收
...
//缓冲区链表结构，放在pool内存池上面
struct ngx_chain_s {
    ngx_buf_t    *buf; //buf数据结构
    ngx_chain_t  *next;//链表数据结构
};

typedef struct {
    ngx_int_t    num;
    size_t       size;
} ngx_bufs_t; //ngx_buf_t缓冲区，通过ngx_chain_t链表结构进行关联和管理
